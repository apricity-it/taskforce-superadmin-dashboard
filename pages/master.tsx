...
//======================= zone ward kothi assignment using firebase ===============================================
...
//   useEffect(() => {
//     const unsubAssignments = DataService.onAssignmentsChange(setAssignments)
//     const unsubFeederPoints = DataService.onFeederPointsChange(setFeederPoints)
//     DataService.getAllUsers().then(users => {
//       setTaskForceUsers(users.filter(u => u.role === 'task_force_team'))
//     })
//     setLoading(false)
//     return () => { unsubAssignments(); unsubFeederPoints() }
//   }, [])

//   const getFeederPointName = (id: string) => feederPoints.find(f => f.id === id)?.name ?? '—'
//   const getUserName = (id: string) => taskForceUsers.find(u => u.id === id)?.name ?? '—'

//   const openAdd = () => { setEditing(null); setFormFeederPointId(''); setFormUserId(''); setShowModal(true) }
//   const openEdit = (a: Assignment) => { setEditing(a); setFormFeederPointId(a.feederPointId); setFormUserId(a.userId); setShowModal(true) }
...
//       const userName = getUserName(formUserId)
//       if (editing) {
//         await DataService.updateAssignment(editing.id, { feederPointId: formFeederPointId, feederPointName, userId: formUserId, userName })
//       } else {
//         await DataService.createAssignment({ feederPointId: formFeederPointId, feederPointName, userId: formUserId, userName })
//       }
//       setShowModal(false)
//     } catch (e) { console.error(e) }
//     setSaving(false)
...
//               >
//                 <option value="">Select a feeder point</option>
//                 {feederPoints.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
//               </select>
//             </div>
//             <div>
//               <label className="block text-sm font-medium text-gray-700 mb-1">Task Force User</label>
