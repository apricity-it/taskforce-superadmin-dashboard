...
export default function FeederPointsPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeZones = DataService.onZonesChange(setAllZones)
    const unsubscribeWards = DataService.onWardsChange(setAllWards)
    const unsubscribeKothis = DataService.onKothisChange(setAllKothis)
  ...
  const filterFeederPoints = () => {
    ...
    // Zone → Ward → Kothi cascade filter
    if (kothiFilter) {
      // Most specific: exact kothi match
      filtered = filtered.filter(fp => fp.kothiId === kothiFilter)
    } else if (wardFilter) {
      // Ward level: match by wardId or kothis that belong to this ward
      const kothiIdsInWard = allKothis
        .filter(k => k.wardId === wardFilter)
        .map(k => k.id)
      filtered = filtered.filter(fp =>
        fp.wardId === wardFilter ||
        kothiIdsInWard.includes(fp.kothiId)
      )
    } else if (zoneFilter) {
      // Zone level: match by zoneId or wards/kothis that fall in this zone
      const wardIdsInZone = allWards
        .filter(w => w.zoneId === zoneFilter)
        .map(w => w.id)
      const kothiIdsInZone = allKothis
        .filter(k => wardIdsInZone.includes(k.wardId))
        .map(k => k.id)
      filtered = filtered.filter(fp =>
        fp.zoneId === zoneFilter ||
        wardIdsInZone.includes(fp.wardId) ||
        kothiIdsInZone.includes(fp.kothiId)
      )
    }
  ...
  const wardOptions = allWards.filter(w => w.zoneId === zoneFilter)
  const kothiOptions = allKothis.filter(k => k.wardId === wardFilter)
    ...
    <div className="space-y-6 scrollable-content">
      ...
      <div className="card">
        <div className="flex flex-col gap-4">
          ...
          {/* Row 1: Search + Status + Assignment */}
          ...
          {/* Row 2: Zone → Ward → Kothi cascade filters */}
          <div className="flex flex-wrap gap-3 items-center">
            ...
            {/* Ward — disabled until zone selected */}
            <select
              value={wardFilter}
              onChange={(e) => {
              ...
              disabled={!zoneFilter}
              className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:bg-gray-50 disabled:text-gray-400 disabled:cursor-not-allowed"
            >
            ...
            {/* Kothi — disabled until ward selected */}
            <select
              value={kothiFilter}
              onChange={(e) => setKothiFilter(e.target.value)}
              disabled={!wardFilter}
              className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:bg-gray-50 disabled:text-gray-400 disabled:cursor-not-allowed"
            >
            ...
            {/* Clear geo filters button */}
            {(zoneFilter || wardFilter || kothiFilter) && (
              <button
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredFeederPoints.map((feederPoint, index) => (
                  <tr
                    key={feederPoint.id || index}
                    className={`hover:bg-gray-50 ${feederPoint.isEliminated ? 'opacity-60 bg-red-50' : ''}`}
                  >
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-500">
                              {feederPoint.assignmentDetails.type === 'individual'
                                ? 'Individual'
                                : `Team (${feederPoint.assignmentDetails.memberCount} members)`}
                            </div>
                          </div>
                        </div>
                      ) : (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">
                {feederPoints.length === 0
                  ? 'No feeder points in database'
                  : 'No feeder points match your filters'}
              </h3>
        ...
        <div className="bg-white rounded-lg shadow-xl transform transition-all sm:max-w-lg sm:w-full">
          <div className="px-4 py-5 sm:p-6">
            ...
            <div className="mt-4 space-y-4">
              ...
              />
              <input
                type="text" name="location.address" value={formData.location?.address || ''} onChange={handleChange}
                placeholder="Address"
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
              />
              <select
...
function FeederPointReportModal({
  ...
  const cleanlinessStatus = formatBooleanAnswer(
    getAnswerFromReports(reports, REPORT_QUESTION_KEYS.cleanliness), 'Clean', 'Not Clean', 'Data Not Available'
  )
  const segregationStatus = formatBooleanAnswer(
    getAnswerFromReports(reports, REPORT_QUESTION_KEYS.segregation), 'Segregated', 'Not Segregated', 'Data Not Available'
  )
  const vehicleStatus = formatBooleanAnswer(
    getAnswerFromReports(reports, REPORT_QUESTION_KEYS.vehicle), 'Present', 'Not Present', 'Data Not Available'
  )
  const nearbyAreaStatus = formatBooleanAnswer(
    getAnswerFromReports(reports, REPORT_QUESTION_KEYS.nearbyArea), 'Yes', 'No', 'Not Documented'
  )
  const signboardStatus = formatBooleanAnswer(
    getAnswerFromReports(reports, REPORT_QUESTION_KEYS.signboard), 'Yes', 'No', 'Not Documented'
  )
  ...
  const finalComplianceScore = formatTitleValue(
    getAnswerFromReports(reports, REPORT_QUESTION_KEYS.compliance), 'Not Available'
  )
  ...
  const [historyStatusFilter, setHistoryStatusFilter] = useState<ReportHistoryFilter>('all')
  ...
  const filteredReportHistory = useMemo(() => {
    if (historyStatusFilter === 'all') return reportHistory
    return reportHistory.filter(item => item.status === historyStatusFilter)
  }, [reportHistory, historyStatusFilter])

  const historyTotalPages = useMemo(() => {
    return Math.max(1, Math.ceil(filteredReportHistory.length / HISTORY_PAGE_SIZE))
  ...
  useEffect(() => { setHistoryPage(1) }, [historyStatusFilter])
          ...
          <div className="mt-6 space-y-6">
            ...
            <section>
              ...
              <div className="mt-3 grid gap-4 text-sm sm:grid-cols-2">
                ...
                <InfoCell label="Date" value={latestReportDate ? latestReportDate.toLocaleDateString() : 'Not Available'} />
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-xs uppercase tracking-wide text-gray-500">
                        {row.photos.length > 0
                          ...
                          : row.hasData ? 'No attachments' : 'Pending'}
                      </span>
                    </div>
                    {row.photos.length > 0 ? (
            ...
            <section>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                ...
                <select
                  ...
                  onChange={e => setHistoryStatusFilter(e.target.value as ReportHistoryFilter)}
                    ...
                    {filteredReportHistory.length > 0 ? (
                      paginatedReportHistory.map(item => {
                        ...
                        return (
                          <tr
                            key={item.key}
                            onClick={() => setSelectedHistoryReport(item.reportRef)}
                ...
                <div className="mt-4 rounded-lg border border-blue-100 bg-blue-50/40 p-4">
                  ...
                  <div className="mt-4 grid gap-4 text-sm text-gray-900 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <span className={`mt-1 inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${selectedReportBadge?.colorClass || 'bg-gray-100 text-gray-700'}`}>
...
function formatBooleanAnswer(value: string | null, positiveLabel: string, negativeLabel: string, fallback: string) {
  ...
  if (YES_ANSWER_VALUES.has(normalized)) return positiveLabel
  if (NO_ANSWER_VALUES.has(normalized)) return negativeLabel
  return value
}
...
function formatWorkerPresence(value: string | null) {
  ...
  if (YES_ANSWER_VALUES.has(normalized)) return 'Present'
  if (NO_ANSWER_VALUES.has(normalized)) return 'Not Present'
  return value
}
...
function getAnswerQuestionLabel(answer: ComplianceAnswer, index: number) {
  if (answer.description && answer.description.trim()) return answer.description.trim()
  if (answer.questionId && answer.questionId.trim()) return humanizeQuestionKey(answer.questionId)
  return `Question ${index + 1}`
}

function getAnswerDisplayValue(answer: ComplianceAnswer) {
  if (answer.answer === null || answer.answer === undefined || answer.answer === '') return 'Not Provided'
...
function buildTripRows(reports: ComplianceReport[]): TripRow[] {
  const sortedReports = [...reports].sort((a, b) => {
    const aTime = getReportPrimaryDate(a)?.getTime() ?? 0
    const bTime = getReportPrimaryDate(b)?.getTime() ?? 0
    return bTime - aTime
  })
  ...
  return TRIP_SEQUENCE.map((tripNumber, index) => {
    ...
    const cleanliness = formatBooleanAnswer(getAnswerFromReport(report, REPORT_QUESTION_KEYS.cleanliness), 'Yes', 'No', 'Yes / No')
    const segregation = formatBooleanAnswer(getAnswerFromReport(report, REPORT_QUESTION_KEYS.segregation), 'Yes', 'No', 'Yes / No')
    const vehicle = formatBooleanAnswer(getAnswerFromReport(report, REPORT_QUESTION_KEYS.vehicle), 'Yes', 'No', 'Yes / No')
...
function collectReportPhotoUrls(report?: ComplianceReport) {
  ...
  report.attachments?.forEach(attachment => {
    if (attachment.type === 'photo' && attachment.url) urls.add(attachment.url)
  })
  report.answers?.forEach(answer => {
    answer.photos?.forEach(photoUrl => { if (photoUrl) urls.add(photoUrl) })
  })
...
function getDistinctTripCount(reports: ComplianceReport[]) {
  ...
  reports.forEach(report => {
    if (typeof report.tripNumber === 'number') tripNumbers.add(report.tripNumber)
  })
  return tripNumbers.size || Math.min(reports.length, TRIP_SEQUENCE.length)
}

function getStatusSummary(reports: ComplianceReport[]) {
  const summary = { total: reports.length, approved: 0, rejected: 0, requiresAction: 0 }
  reports.forEach(report => {
    if (report.status === 'approved') summary.approved += 1
    else if (report.status === 'rejected') summary.rejected += 1
    else if (report.status === 'requires_action') summary.requiresAction += 1
  })
  return summary
}
...
function getStatusBadge(status?: ComplianceReport['status']) {
  switch (status) {
    case 'approved': return { label: 'Approved', colorClass: 'bg-green-100 text-green-700' }
    case 'rejected': return { label: 'Rejected', colorClass: 'bg-red-100 text-red-700' }
    case 'requires_action': return { label: 'Requires Action', colorClass: 'bg-orange-100 text-orange-700' }
    case 'pending': return { label: 'Pending', colorClass: 'bg-yellow-100 text-yellow-700' }
    default: return { label: 'Unknown', colorClass: 'bg-gray-100 text-gray-700' }
  }
}
