...
export default function PointsMapLeaflet({ points, reports = [], dark }: PointsMapProps) {
  ...
  const handleSelect = useCallback((p: FeederPoint) => {
    setSelectedPoint(p)
    if (mapRef.current && p.location?.latitude && p.location?.longitude) {
      ...
      <div className="pmw">
        ...
        <div className="stats-row">
          {[
            ...
            { label: 'Feeder',     val: stats.feeder,    color: '#3b82f6' },
            { label: 'Chronic',    val: stats.chronic,   color: '#ef4444' },
            { label: 'Active',     val: stats.active,    color: '#22c55e' },
            { label: 'Inactive',   val: stats.inactive,  color: '#ef4444' },
            { label: 'Eliminated', val: stats.eliminated,color: '#a855f7' },
          ].map(s => (
        ...
        <div className="map-row">
          ...
          {selectedPoint && (
            <DetailPanel
              point={selectedPoint}
              pointStats={pointStatsMap[selectedPoint.id] ?? { total: 0, approved: 0, pending: 0, rejected: 0 }}
              dark={dark}
